...
public final class Scan {
	...
	private Stack<Tuple<Token, Integer>> bracketStack; // 
	...
	private void pushBracket(Tuple<Token, Integer> tuple) {
		Assertion.check()
				.isNotNull(tuple);
		//---
		//---Brackets define blocks 
		if (Lexicon.isLeftBracket(tuple.val1())) {
			bracketStack.push(tuple);
		} else {
			final var last = bracketStack.pop();
			//an ending bracket must follow an opening bracket ]=>[ ; }=>{ ; )=>(
			if (!Lexicon.isPairOfBrackets(last.val1(), tuple.val1())) {
				throw buildException(last, "a block is not well formed");
			}
		}
	}
