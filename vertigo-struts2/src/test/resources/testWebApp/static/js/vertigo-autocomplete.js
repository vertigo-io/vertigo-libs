	...
	$(myAutocompleteName).keyup(function(event) {
		if ($(myAutocompleteKeyName).val() != "") {			
	        if (!event.ctrlKey && !event.metaKey && !event.altKey && event.which != 13) {
	        	$(myAutocompleteKeyName).val(null);
	        	$(myAutocompleteName).val(null);
	        }
		}
	});
	
	$(myAutocompleteName).autocomplete({
		select: function( event, ui ) {
			if(ui.item){
	            $(myAutocompleteKeyName).val(ui.item.key);
	            $(myAutocompleteName).val(ui.item.value);
	            return false;//on stop l'event
	        } else {
...
function formatAutocomplete(itemValue, term) {
	...
	var normalizedLabel = removeDiacritics(itemValue.toUpperCase());
	var normalizedQuery = removeDiacritics(term.toUpperCase()).replace(/\s*/, " ");
	//remove accents
	var highlightedLabel = sLabel;
	var highlightedNormalizedLabel = normalizedLabel;
	var searchQueries = normalizedQuery.split(" ");
	for ( var i = 0; i < searchQueries.length; i++) {
		var searchQuery = searchQueries[i];
		searchQuery = searchQuery.replace(/^\s+|\s+$/g, '');
		if(searchQuery.length > 0) {
			var queryIndex = (" "+highlightedNormalizedLabel).indexOf(" "+searchQuery);
			if(queryIndex!=-1) {
				highlightedLabel = highlightText(highlightedLabel, queryIndex, searchQuery.length);
				highlightedNormalizedLabel = highlightText(highlightedNormalizedLabel, queryIndex, searchQuery.length);
			}
		}
	}
...
function highlightText(sText, iIndex, iLenght) {
	var highlightedLabel = [sText.substring(0, iIndex),
		"<b class='highlight'>",
	   	sText.substring(iIndex, iIndex+iLenght),
	   	"</b>",
	   	sText.substring(iIndex+iLenght)];
	return (highlightedLabel.join(""));
}
