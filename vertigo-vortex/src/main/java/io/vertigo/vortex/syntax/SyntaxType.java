...
import io.vertigo.vortex.tokenizer.TokenType;

public enum SyntaxType implements TokenType {
	...
	//word must be placed AFTER bool 
	...
	public String extract(String value) {
		...
		return switch (this) {
			...
			case string_basic -> value
					.substring(1, value.length() - 1)
					.replace("\\\\", "\\") //   // -> /
					.replace("\\\"", "\""); //  /" -> "
			case string_basic_multi_line -> value
					.substring(3, value.length() - 3)
					.replace("\\\\", "\\") //   // -> /
					.replace("\\\"", "\""); //  /" -> "
			case string_strict -> value
					.substring(1, value.length() - 1);
			case string_strict_multi_line -> value
					.substring(3, value.length() - 3);
			default -> value;
		};
	}
