	...
	private Map<String, Object> parseIndexType(final String[] indexTypeArray, final String indexType) {
		...
		for (int i = 1; i < indexTypeArray.length; i++) {
			...
			if (INDEX_STORED.equals(indexTypeParam) || INDEX_NOT_STORED.equals(indexTypeParam)) {
				Assertion.check().isTrue(!parsedIndexType.containsKey(INDEX_STORED_KEY), INDEX_TYPE_ERROR_MSG, indexType);
				...
				Assertion.check().isTrue(!parsedIndexType.containsKey(INDEX_SUB_KEYWORD_KEY), INDEX_TYPE_ERROR_MSG, indexType);
				...
				Assertion.check().isTrue(!parsedIndexType.containsKey(INDEX_FIELD_DATA_KEY), INDEX_TYPE_ERROR_MSG, indexType);
				parsedIndexType.put(INDEX_FIELD_DATA_KEY, INDEX_FACETABLE.equals(indexTypeParam));
			} else {
				Assertion.check().isTrue(!parsedIndexType.containsKey(INDEX_DATA_TYPE_KEY), INDEX_TYPE_ERROR_MSG, indexType);
