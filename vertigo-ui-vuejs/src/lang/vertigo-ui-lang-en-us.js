export default {
		...
		uploader : {
			...
			fileErrorTooBigSize: (size) => `A single file must not exceed ${Math.round(size/1024/1024)} MB`,
			fileErrorMaxTotalSize: (size) => `The total file size must not exceed ${Math.round(size/1024/1024)} MB`,
			fileErrorAccept: (accept) => `The file type is not allowed, only '${accept}' files are allowed`,
			fileCountError: (max) => `The field can contain a maximum of ${max} document${max>1?'s':''}`,
			progress : "Progress",
