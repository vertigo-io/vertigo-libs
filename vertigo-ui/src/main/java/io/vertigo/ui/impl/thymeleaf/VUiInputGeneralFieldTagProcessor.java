 ...
 *
 */
public final class VUiInputGeneralFieldTagProcessor
	...
	private static final String[] ALL_TYPE_ATTR_VALUES = new String[] {
			null,
	...
	public VUiInputGeneralFieldTagProcessor(final String dialectPrefix) {
	...
	protected void doProcess(final ITemplateContext context,
		...
		final String type = tag.getAttributeValue(typeAttributeDefinition.getAttributeName());
		...
		final String value = applyConversion(type) ? SpringValueFormatter.getDisplayString(bindStatus.getValue(), bindStatus.getEditor(), true) : SpringValueFormatter.getDisplayString(bindStatus.getActualValue(), true);

		StandardProcessorUtils.setAttribute(structureHandler, idAttributeDefinition, ID_ATTR_NAME, id); // No need to escape: this comes from an existing 'id' or from a token
		StandardProcessorUtils.setAttribute(structureHandler, nameAttributeDefinition, NAME_ATTR_NAME, name); // No need to escape: this is a java-valid token

		StandardProcessorUtils.setAttribute(
				structureHandler, valueAttributeDefinition, VALUE_ATTR_NAME, RequestDataValueProcessorUtils.processFormFieldValue(context, name, value, type));
