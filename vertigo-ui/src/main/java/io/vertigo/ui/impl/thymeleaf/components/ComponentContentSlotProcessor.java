...
public class ComponentContentSlotProcessor extends AbstractElementModelProcessor {
	 ...
	 */
	public ComponentContentSlotProcessor(final String dialectPrefix) {
	...
	protected void doProcess(final ITemplateContext context, final IModel model, final IElementModelStructureHandler structureHandler) {
		...
		Assertion.checkArgument(attributeValue.endsWith(ComponentSlotAttributeTagProcessor.VARIABLE_PLACEHOLDER_SEPARATOR + ComponentSlotAttributeTagProcessor.SLOTS_SUFFIX), "{0} isn't a slot. Tag vu:content-slot supports only slots, names must ends with '_slot'", attributeValue);
		//-----
