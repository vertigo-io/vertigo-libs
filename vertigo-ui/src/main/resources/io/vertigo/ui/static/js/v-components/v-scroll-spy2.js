...
Vue.directive('scroll-spy', {
        bind: function(elNav, args) {
        	...
        	const scannerLine2 = document.createElement("HR");  
        	scannerLine2.style.position='absolute';
        	scannerLine2.style.top=scanner+'px';
        	scannerLine2.style.border='none';
        	scannerLine2.style.borderTop='green solid 1px';
        	scannerLine2.style.width='100%';
        	scannerLine2.style.zIndex='10000';
        	document.querySelector('body').appendChild(scannerLine2);
        	
        	const scannerLine3 = document.createElement("HR");  
        	scannerLine3.style.position='absolute';
        	scannerLine3.style.top=scanner+'px';
        	scannerLine3.style.border='none';
        	scannerLine3.style.borderTop='blue solid 1px';
        	scannerLine3.style.width='100%';
        	scannerLine3.style.zIndex='10000';
        	document.querySelector('body').appendChild(scannerLine3);
        	...
        	Vue.computeBreakPoints = function(scrollPosition){
        		...
        		const scrollEnd = blockHeight[blockHeight.length-1]+30; /** Last element height */
        		const scrollStart = blockHeight[0]+30; /** Start linear move at this scroll position */
        		
				var scrollBreakpoints = []
        		scrollBreakpoints.push(0)
        		for(var i = 1 ; i < elAs.length; i++) {
        			//scrollBreakpoints[i] = scrollStart + (blockHeight[i]) * (scrollMax-scrollStart-scanner)
        			scrollBreakpoints[i] = ((blockHeight[i]-scrollStart) * scrollMax)/(scrollEnd-scrollStart);
        			scrollBreakpoints[i] = Math.round(scrollBreakpoints[i])
        			console.log(i+'  from: '+blockHeight[i]+ '  to:'+scrollBreakpoints[i] )
        		}
        		
				scannerLine1.style.top=scrollBreakpoints[1]+'px';
        		scannerLine2.style.top=scrollBreakpoints[2]+'px';
        		scannerLine3.style.top=scrollBreakpoints[3]+'px';
